...
class CBConic : public GRBCallback {
public:
    int products;
    int customers;
    vector<double> noPay;
    ...
    GRBVar* y;
    CBConic();
    CBConic(GRBVar* x, GRBVar* y, int products, int customers, vector<double> noPay, vector<vector<double>> util, vector<vector<double>> ren);
protected:
    void callback();
};
...
public:
    ...
    char var_name[1000];
    BCConic();
    BCConic(Data data, double time_limit, string outfile);
