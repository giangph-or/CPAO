...
double CEFlog::calculate_optimal_bound_denominator(Data data, int i) {
	...
	GRBLinExpr obj;
	for (int j = 0; j < data.number_products; ++j)
		obj += round(data.utilities[i][j] * 100) * x[j];
	...
	return model.get(GRB_DoubleAttr_ObjVal) + round(data.no_purchase[i] * 100);
}
...
void CEFlog::solve(Data data) {
	...
	for (int i = 0; i < data.number_customers; ++i) {
		double sum_uti = 0;
		for (int j = 0; j < data.number_products; ++j)
			sum_uti += round(data.fraction[i] * 100) * round(data.utilities[i][j] * 100) * round((alpha[i] - data.revenue[i][j]) * 100);
...
//cout << "Slack variables : s_i\n" << endl;
	...
	for (int i = 0; i < data.number_customers; ++i) {
		...
		for (int k = 1; k <= index_w[i]; ++k)
			sum += pow(2, k - 1) * z[i][k];
		model.addConstr(t[i] >= sum + round(data.fraction[i] * 100) * round(data.no_purchase[i] * 100) * round(alpha[i] * 100) * y[i]);
	}
	...
	for (int i = 0; i < data.number_customers; ++i) {
		GRBLinExpr sum_x = 0;
		for (int j = 0; j < data.number_products; ++j)
			sum_x += round(data.utilities[i][j] * 100) * x[j];
		model.addConstr(r[i] == sum_x + round(data.no_purchase[i] * 100));
	}
	...
	for (int i = 0; i < data.number_customers; ++i) {
		...
		for (int j = 0; j < data.number_products; ++j)
			sum_x += round(data.fraction[i] * 100) * round(data.utilities[i][j] * 100) * round((alpha[i] - data.revenue[i][j]) * 100) * x[j];
	...
	for (int i = 0; i < data.number_customers; ++i)
		for (int k = 1; k <= index_w[i]; ++k) {
			...
			model.addConstr(z[i][k] >= y[i] + (w[i][k] - 1) * upper_bound_y[i]);
		}

	////cout << "Polymatroid constraints" << endl;
	//for (int i = 0; i < data.number_customers; ++i) {
	//	GRBQuadExpr sum = round(data.fraction[i] * 100) * round(data.no_purchase[i] * 100) * round(alpha[i] * 100);
	//	for (int k = 1; k <= index_w[i]; ++k)
	...
	for (int i = 0; i < data.number_customers; ++i) {
		...
		gamma[0] = round(data.fraction[i] * 100) * round(data.no_purchase[i] * 100) * round(alpha[i] * 100);
		for (int k = 1; k <= index_w[i]; ++k)
