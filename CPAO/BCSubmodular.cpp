...
BCSubmodular::BCSubmodular() {

}

BCSubmodular::BCSubmodular(Data data, double time_limit, string outfile) {
	this->data = data;
	this->time_limit = time_limit;
	this->out_res_csv = outfile;
}

CBSubmodular::CBSubmodular() {

}

CBSubmodular::CBSubmodular(GRBVar* varx, GRBVar* vary, int p, int c, vector<double> n, vector<vector<double>> u, vector<vector<double>> r) {
	x = varx;
	y = vary;
	products = p;
	customers = c;
...
double BCSubmodular::calculate_z(Data data, vector<double> alpha, vector<double> denominator) {
	double z = 0;
...
double BCSubmodular::calculate_optimal_bound_denominator(Data data, int i) {
...
double BCSubmodular::calculate_master_obj(Data data, vector<int> x) {
	double obj = 0;
	for (int i = 0; i < data.number_customers; ++i) {
...
vector<vector<double>> BCSubmodular::calculate_bound_y_in(Data data) {
...
vector<vector<double>> BCSubmodular::calculate_bound_y_notin(Data data) {
...
vector<vector<double>> BCSubmodular::subset_bound_y_in(Data data) {
	vector<vector<double>> lb_in(data.number_customers);
	for (int i = 0; i < data.number_customers; ++i)
		lb_in[i].resize(data.number_products);
...
vector<vector<double>> BCSubmodular::subset_bound_y_notin(Data data) {
	vector<vector<double>> lb_notin(data.number_customers);
	for (int i = 0; i < data.number_customers; ++i)
		lb_notin[i].resize(data.number_products);
...
void BCSubmodular::solve_multicut_psi(Data data) {
	...
	CBSubmodular cb = CBSubmodular(x, y, data.number_products, data.number_customers, data.no_purchase, data.utilities, data.revenue);
