...
CuttingPlaneNCuts::CuttingPlaneNCuts() {

}

CuttingPlaneNCuts::CuttingPlaneNCuts(Data data, double time_limit, string outfile) {
	this->data = data;
	this->time_limit = time_limit;
	this->out_res_csv = outfile;
}

vector<double> CuttingPlaneNCuts::calculate_y(Data data, vector<int> x, vector<double> alpha) {
...
vector<double> CuttingPlaneNCuts::calculate_z(Data data, vector<int> x) {
...
double  CuttingPlaneNCuts::calculate_original_obj(Data data, vector<int> x, vector<double> alpha) {
	double obj = 0;
	for (int i = 0; i < data.number_customers; ++i) {
...
double CuttingPlaneNCuts::calculate_master_obj(Data data, vector<int> x) {
	double obj = 0;
	for (int i = 0; i < data.number_customers; ++i) {
...
double CuttingPlaneNCuts::calculate_original_obj_tmp(Data data, vector<int> x, vector<double> alpha, int candidate) {
	double obj = 0;
	for (int i = 0; i < data.number_customers; ++i) {
...
vector<int> CuttingPlaneNCuts::greedy(Data data, int budget, vector<double> alpha) {
...
double CuttingPlaneNCuts::calculate_bound_y(Data data, int budget, int i, double alpha) {
	double sum = 0;
...
double CuttingPlaneNCuts::calculate_bound_z(Data data, int budget, int i) {
	double sum = 0;
	vector<double> u(data.number_products);
	for (int j = 0; j < data.number_products; ++j)
...
double CuttingPlaneNCuts::calculate_optimal_bound_y(Data data, int budget, int i, double alpha) {
...
void CuttingPlaneNCuts::solve_build_in(Data data, int budget, int nCuts) {
	...
	for (int c = 0; c < nCuts; ++c)
		theta[c] = model.addVar(-GRB_INFINITY, GRB_INFINITY, 0, GRB_CONTINUOUS, "theta_" + to_string(c));
