...
class CBMulticutSen : public GRBCallback {
public:
    int products;
    int customers;
    int cuts;
    ...
    CBMulticutSen();
    CBMulticutSen(GRBVar* x, GRBVar* y, GRBVar* cut_customers, int products, int customers, vector<double> noPay, vector<vector<double>> util, vector<vector<double>> ren, vector<double> al, vector<double> frac, int cuts);
protected:
    void callback();
};
...
public:
    ...
    char var_name[1000];
    BCMulticutSen();
    BCMulticutSen(Data data, double time_limit, string outfile);
